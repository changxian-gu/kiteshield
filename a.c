...
int verify(uint8_t* data) {
    ...
    uint16_t CRC16 = CRC16_Check(data, 4 + 32);
    if ((CRC16 >> 8) != data[36] || (CRC16 & 0xFF) != data[37]) {
        printf("CRC16 error\n");
        return -1;
    }
    if (data[38] != 0xFF)
...
int main() {
  ...
  srand(time(NULL));
  for (int i = 0; i < 32; i++) {
    random[i] = rand() % 256; // 生成 0 到 255 之间的随机数
  }
