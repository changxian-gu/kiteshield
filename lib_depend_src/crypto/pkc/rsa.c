   ...
   size_t k, uint_t e, RsaPrivateKey *privateKey)
{
   ...
   do
   {
      do
      {
         ...
         error = mpiCheckProbablePrime(&privateKey->p, prngContext);
   ...
   do
   {
      do
      {
         ...
         error = mpiCheckProbablePrime(&privateKey->q, prngContext);
...
void rsaPrivateKeyFormat(const RsaPrivateKey* privateKey, uint8_t* output, FormatedRsaPrivateKey* formatedPrivateKey) {
   uint8_t* p = output;
   formatedPrivateKey->nLen = mpiGetByteLength(&privateKey->n);
   mpiExport(&privateKey->n, p, formatedPrivateKey->nLen, MPI_FORMAT_LITTLE_ENDIAN);
   p += formatedPrivateKey->nLen;

   formatedPrivateKey->eLen = mpiGetByteLength(&privateKey->e);
   mpiExport(&privateKey->e, p, formatedPrivateKey->eLen, MPI_FORMAT_LITTLE_ENDIAN);
   p += formatedPrivateKey->eLen;

   formatedPrivateKey->dLen = mpiGetByteLength(&privateKey->d);
   mpiExport(&privateKey->d, p, formatedPrivateKey->dLen, MPI_FORMAT_LITTLE_ENDIAN);
   p += formatedPrivateKey->dLen;

   formatedPrivateKey->pLen = mpiGetByteLength(&privateKey->p);
   mpiExport(&privateKey->p, p, formatedPrivateKey->pLen, MPI_FORMAT_LITTLE_ENDIAN);
   p += formatedPrivateKey->pLen;

   formatedPrivateKey->qLen = mpiGetByteLength(&privateKey->q);
   mpiExport(&privateKey->q, p, formatedPrivateKey->qLen, MPI_FORMAT_LITTLE_ENDIAN);
   p += formatedPrivateKey->qLen;

   formatedPrivateKey->dpLen = mpiGetByteLength(&privateKey->dp); 
   mpiExport(&privateKey->dp, p, formatedPrivateKey->dpLen, MPI_FORMAT_LITTLE_ENDIAN);
   p += formatedPrivateKey->dpLen;

   formatedPrivateKey->dqLen = mpiGetByteLength(&privateKey->dq);
   mpiExport(&privateKey->dq, p, formatedPrivateKey->dqLen, MPI_FORMAT_LITTLE_ENDIAN);
   p += formatedPrivateKey->dqLen;

   formatedPrivateKey->qinvLen = mpiGetByteLength(&privateKey->qinv);
   mpiExport(&privateKey->qinv, p, formatedPrivateKey->qinvLen, MPI_FORMAT_LITTLE_ENDIAN);

   // int len = 0;
   // len = formatedPrivateKey->nLen + formatedPrivateKey->eLen + formatedPrivateKey->dLen + formatedPrivateKey->pLen + formatedPrivateKey->qLen + formatedPrivateKey->dpLen + formatedPrivateKey->dqLen + formatedPrivateKey->qinvLen;
   // ks_printf(1, "len : %d\n", len);
}

void rsaPrivateKeyParse(FormatedRsaPrivateKey* formatedRsaPrivateKey, RsaPrivateKey* privateKey) {
   uint8_t* p = formatedRsaPrivateKey->data;
   mpiImport(&privateKey->n, p, formatedRsaPrivateKey->nLen, MPI_FORMAT_LITTLE_ENDIAN);
   p += formatedRsaPrivateKey->nLen;

   mpiImport(&privateKey->e, p, formatedRsaPrivateKey->eLen, MPI_FORMAT_LITTLE_ENDIAN);
   p += formatedRsaPrivateKey->eLen;

   mpiImport(&privateKey->d, p, formatedRsaPrivateKey->dLen, MPI_FORMAT_LITTLE_ENDIAN);
   p += formatedRsaPrivateKey->dLen;

   mpiImport(&privateKey->p, p, formatedRsaPrivateKey->pLen, MPI_FORMAT_LITTLE_ENDIAN);
   p += formatedRsaPrivateKey->pLen;

   mpiImport(&privateKey->q, p, formatedRsaPrivateKey->qLen, MPI_FORMAT_LITTLE_ENDIAN);
   p += formatedRsaPrivateKey->qLen;

   mpiImport(&privateKey->dp, p, formatedRsaPrivateKey->dpLen, MPI_FORMAT_LITTLE_ENDIAN);
   p += formatedRsaPrivateKey->dpLen;

   mpiImport(&privateKey->dq, p, formatedRsaPrivateKey->dqLen, MPI_FORMAT_LITTLE_ENDIAN);
   p += formatedRsaPrivateKey->dqLen;
   
   mpiImport(&privateKey->qinv, p, formatedRsaPrivateKey->qinvLen, MPI_FORMAT_LITTLE_ENDIAN);
}
