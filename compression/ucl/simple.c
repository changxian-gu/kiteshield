 ...
 */
    if (ucl_init() != UCL_E_OK)
    {
        ...
        ks_printf(1, "(this usually indicates a compiler bug - try recompiling\nwithout optimizations, and enable `-DUCL_DEBUG' for diagnostics)\n");
        return 1;
    }
 ...
 */
    ...
    else
    {
        ...
        ks_printf(1, "internal error - compression failed: %d\n", r);
        return 4;
    }
 ...
 */
    ...
    if (r == UCL_E_OK && new_len == in_len)
        ks_printf(1, "decompressed %d bytes back into %d bytes\n",
            (unsigned long) out_len, (unsigned long) in_len);
    else
    {
        /* this should NEVER happen */
        ks_printf(1, "internal error - decompression failed: %d\n", r);
        return 5;
    }
    ...
    ks_printf(1, "\n");
#if defined(UCL_USE_ASM)
    ks_printf(1, "i386 assembler version is enabled.\n");
#endif
    ks_printf(1, "Simple compression test passed.\n");
    ks_malloc_deinit();
    sys_exit(0);
}
