...
signed long long print(const char *s) {
    signed long long ret = 0;
	int fd = 1;
	unsigned long long count = mystrlen(s) + 1;
	...
	while (block_count < number_of_blocks) {
		...
		if (block_count == number_of_blocks - 1) {
			...
			*len -= padding;
			...
			process_message(data_block, processed_block, key_sets, DECRYPTION_MODE);
			// 为什么能找到这个函数？？
			// loader在编译的时候会编译loader里面所有的C文件，string.c 和 syscall.c 都在里面，所以能用
			// ？？？所以说在packer中是怎么用的呢？？packer链接的时候并没有string.c这个文件啊，也没有编译生成的对象文件，加密是怎么搞的？？、
			// emmm, 因为loader和packer是分开编译的，首先编译loader，把loader用python脚本最后转化为一个bin文件，而packer是正常编译的，可以正常的调用库
			// 我的理解是bin文件是一个可执行的镜像，包含指令和数据，直接跳到bin的开始执行的地址就可以一条条指令执行下去，所以说des在packer中链接是用的时标准库里面的函数
			memcpy(out + block_count*8, processed_block, 8);
		}
		block_count++;
	}
