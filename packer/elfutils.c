...
void parse_mapped_elf(void *start, size_t size, struct mapped_elf *elf) {
    /**
     * 构建elf映射
     * start是申请空间的头地址
     */
     ...
     * 常见的节名为”.strtab”或”.shstrtab”。这两个字符串表分别为字符串表(String
     * Table) 和节表字符串表(Header String
     * Table)，字符串表保存的是普通的字符串，
...
const Elf64_Shdr *elf_get_sec_by_name(const struct mapped_elf *elf,
                                      const char *name) {
    /**
...
const char *elf_get_sec_name(const struct mapped_elf *elf,
     ...
     * + sh_name 下标取值
     */
    return (const char *)(elf->start + elf->shstrtab->sh_offset +
                          shdr->sh_name);
}

uint8_t *elf_get_sym_location(const struct mapped_elf *elf,
                              const Elf64_Sym *sym) {
...
int elf_sym_in_text(const struct mapped_elf *elf, const Elf64_Sym *sym) {
    return elf->text->sh_addr <= sym->st_value &&
           (elf->text->sh_addr + elf->text->sh_size) > sym->st_value;
}

const char *elf_get_sym_name(const struct mapped_elf *elf,
                             const Elf64_Sym *sym) {
